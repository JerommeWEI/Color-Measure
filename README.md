# Color-Measure

Color-Measure 是一个基于 Python `tkinter` 的光学与色度学参数计算工具。当前版本为 `v1.7`。

## 运行方式

```powershell
python main.py
```

也可以在 Windows 下使用 `pythonw.exe main.py` 启动，避免额外打开控制台窗口。

## 项目结构

入口文件 `main.py` 位于根目录，功能按职责拆分到 `src/` 包内：

- `src/constants.py`：主题、字体、光谱范围、CIE 1931 配色函数表、白点等常量
- `src/io_utils.py`：输入解析、反射率与 FWHM 标定 CSV 读取、插值、采样波长生成
- `src/optics.py`：物高 / 工作距离几何光学计算
- `src/colorimetry.py`：CIE 插值、XYZ/Lab、CIEDE2000、FWHM 高斯采样、通道漂移与场偏差分析
- `src/widgets.py`：自绘圆角按钮等控件
- `src/app.py`：Tk 主窗口与回调编排

## v1.1 功能

### 物高计算

用于根据目标尺寸和光学参数估算工作距离、观察口径和物面分辨能力。

支持输入：

- 目标长度
- 目标宽度
- FOV 半视场角
- 入瞳直径
- 焦距

输出内容：

- 目标对角线
- 对角线半高
- 工作距离
- 2°观察角观测口径
- 中心物点可探测半锥角
- 中心物点可探测全锥角
- F/#
- 像面 Rayleigh 极限
- 物面 Rayleigh 极限
- 衍射极限等效 DPI
- 目标长度方向可分辨点数
- 目标宽度方向可分辨点数

### 色度学计算

用于评估光谱相机各通道中心波长（CWL）整体随机漂移对色度学结果的影响（蒙特卡洛模拟）。

支持输入：

- 反射率 CSV 文件
- 采样起始波长
- 采样结束波长
- 采样间隔
- 通道漂移幅度（每通道独立均匀分布 ±D）
- 模拟次数（蒙特卡洛场景数）
- 随机种子（固定种子可复现）
- Lab 白点
- FWHM 模型
- 固定 FWHM
- FWHM 面阵标定 CSV
- FWHM 视场位置

计算逻辑：

- 以实测反射率曲线（完整连续色块反射光谱）为输入
- 按设定波段和间隔生成采样通道（CWL）
- 每个通道以带宽（FWHM）高斯光谱响应对连续反射光谱加权积分，得到通道反射率
- 蒙特卡洛整体漂移：每次给所有通道各抽一个独立 U[-D,+D] 漂移量，整体重算 Lab，对每个样品计算相对基准的 ΔE00，重复 N 次（后台线程计算，状态栏显示进度）
- 固定随机种子，相同输入结果完全可复现
- FWHM 来源为「固定 FWHM」或「面阵标定表」二选一（漂移 ΔE00 不支持「忽略 FWHM」）
- 导入面阵标定表时，过滤非 `ok` ROI 并按视场位置选择有效 FWHM
- 以中心 ROI 的 FWHM 为基准，计算中心到边缘 FWHM 变化造成的 Delta E00 偏差
- 使用 CIEDE2000 计算漂移前后的 Delta E00

输出内容：

- 每个样品的平均 Delta E00
- 每个样品的最大 Delta E00
- 每个样品的 P95 Delta E00
- 全局平均 Delta E00
- 全局最大 Delta E00
- 全局 P95 / P99 Delta E00
- 最差样品
- FWHM 场偏差的平均 Delta E00
- FWHM 场偏差的最大 Delta E00
- FWHM 场偏差 Top 5 ROI

### 界面

- 选项卡被选中后的视觉比例调整为未选中状态的 90%。

## 计算说明

本项目运行时依赖 Python 标准库和 [numpy](https://numpy.org/)（用于色度学蒙特卡洛模拟的向量化加速）。色度学计算内置 CIE 1931 2° 配色函数表和 CIEDE2000 计算公式。

## 开发与测试

运行时依赖见 `requirements.txt`；测试使用 pytest（仅开发依赖）：

```powershell
python -m pytest tests/
```

测试覆盖物高计算、色度学核心（含 CIEDE2000 官方参考算例）、CSV 读取与插值等纯函数。

## 版本

- `v1.7`: 引入 D50/D65/A 标准光源光谱功率分布(SPD)，修复 Lab 白点切换结果不变的既存缺陷（样品 XYZ 现按 reflectance·SPD·配色函数积分，参考白为完全漫反射体），不同光源结果真正不同。
- `v1.6`: 窗口高度再增大 30%（1300×1048），并为色度学「计算过程」日志框增加垂直滚动条，确保长日志完整可读。
- `v1.5`: 修复蒙特卡洛向量化在浮点随机种子下崩溃的问题（v1.4 回归，numpy 种子需 int），补充浮点种子回归测试。
- `v1.4`: 选项卡高度缩减 30%、窗口放大 30%；色度学选项卡重构（配置参数可折叠，计算基准/按钮/进度条移至右侧）；蒙特卡洛用 numpy 向量化加速。
- `v1.3`: 修复 FWHM 高斯积分在数据边界附近的浮点累积越界问题，补充对应回归测试。
- `v1.2`: 将反射率与 FWHM 标定的默认路径改为空，避免依赖本机绝对路径，提升可移植性；用户选择后由本地设置记住。
- `v1.1`: 增加色度学 FWHM 固定值与面阵标定表配置，计算中心到边缘 FWHM 场偏差；调整选中选项卡视觉比例为未选中状态的 90%。
- `v1.0`: 初始 GUI 版本，包含物高/工作距离计算和色度学通道漂移敏感度分析。
